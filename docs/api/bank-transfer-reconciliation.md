...
Each selected bank transaction must be `UNRECONCILED` and identify exactly one active receiving bank account. Its processing status does not prevent admin reconciliation. The manual policy intentionally does not require an individual amount or the total selected amount to equal the tuition payment. The backend reuses the newest pending manual attempt for the same intent when one exists, regardless of QR expiry; otherwise it creates a manual `PaymentAttempt` with `SUCCEEDED`. A transaction already linked to an attempt of the same intent may also be confirmed late by an admin, regardless of that attempt's QR expiry or confirmation mode. It then marks each selected bank transaction `MATCHED` plus `reconciliationStatus: ADMIN`. Duplicate IDs and transactions linked to another tuition payment are rejected; any failure rolls back the whole action. The admin action is audited.
