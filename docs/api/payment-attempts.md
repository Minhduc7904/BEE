...
| Side effect       | Tạo `PaymentAttempt` khi chưa có attempt dùng được hoặc attempt gần hết hạn; chỉ tạo `PaymentIntent` khi xử lý dữ liệu cũ chưa có intent. Attempt cũ vẫn được giữ làm lịch sử. |
...
| Side effect       | Tạo attempt/QR mới trong cùng transaction; không thay đổi trạng thái các attempt cũ. |
...
Response là `BaseResponseDto<PaymentInstructionResponseDto>` có đúng shape của endpoint GET, nhưng `paymentAttemptId`, `attemptCode`, `qrCodeUrl` và `expiresAt` là của attempt mới. QR/mã cũ vẫn có thể được auto-match cho đến hết hạn tự nhiên của chính attempt đó.
