...
| Bấm **Làm mới** / **Tạo QR mới**                      | `POST /api/tuition-payments/my/:paymentId/payment-instructions/refresh`                                                | Endpoint tạo QR/attempt mới; QR cũ vẫn hợp lệ đến hết hạn tự nhiên của chính QR đó.                 |
