...
export class CompetitionInfoDto {
  ...
  static fromEntity(competition: Competition): CompetitionInfoDto {
    return {
      ...
      durationMinutes: competition.getDurationMinutes() ?? undefined,
      isUnlimited: competition.isUnlimited(),
...
export class CompetitionExamQuestionDto {
  ...
  static fromEntity(
    ...
    return {
      ...
      sectionId,
      points,
      statements: question.statements ? CompetitionExamStatementDto.fromEntities(question.statements) : undefined,
    }
  }
}
...
export class CompetitionExamSectionDto {
  ...
  static fromEntities(sections: Section[]): CompetitionExamSectionDto[] {
    return sections.sort((a, b) => a.order - b.order).map((s) => CompetitionExamSectionDto.fromEntity(s))
  }
}
...
export class CompetitionExamDto {
  ...
  static fromEntity(exam: Exam): CompetitionExamDto {
    ...
    const sections = exam.sections ? CompetitionExamSectionDto.fromEntities(exam.sections) : []
    ...
    const questionsWithoutSection =
      exam.questions
        ...
        .sort((a, b) => a.order - b.order)
        .map((qe) =>
          CompetitionExamQuestionDto.fromEntity(qe.question!, qe.order, undefined, qe.points ?? undefined),
        ) ?? []
    ...
    if (isTrueFalse) {
      ...
      if (trueFalseMap !== null) {
        ...
        trueFalseAnswers = allStatementIds.map((statementId) => ({
          statementId,
          isTrue: statementId.toString() in trueFalseMap ? trueFalseMap[statementId.toString()] : null,
        }))
        ...
        isTrueFalseAnswered =
          hasSubmitted && trueFalseAnswers.length > 0 && trueFalseAnswers.every((a) => a.isTrue !== null)
      }
    }
