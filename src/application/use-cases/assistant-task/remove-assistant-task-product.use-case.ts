...
@Injectable()
export class RemoveAssistantTaskProductUseCase {
  constructor(
    @Inject('UNIT_OF_WORK') private readonly uow: IUnitOfWork,
    private readonly submissionService: AssistantTaskProductSubmissionService,
  ) {}

  async execute(assistantTaskId: number, assistantTaskProductId: number, actorAdminId: number) {
    await this.uow.executeInTransaction(async (repos) => {
      const removed = await this.submissionService.remove(repos, assistantTaskId, assistantTaskProductId)
    ...
    return BaseResponseDto.success('Gỡ sản phẩm khỏi công việc thành công')
  }
}
