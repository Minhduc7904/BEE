...
export class SendAttendanceToParentUseCase {
    ...
    async execute(input: SendAttendanceToParentInput): Promise<boolean> {
        const appId =
            ...
            SendAttendanceToParentUseCase.DEFAULT_APP_ID
        ...
        const className =
            attendance.classSession?.courseClass?.className || 'N/A'
            ...
            if (typeof homeworkId === 'number' && studentId) {
                const homeworkSubmit =
                    await this.unitOfWork.executeInTransaction(async (repos) => {
                        return repos.homeworkSubmitRepository.findByHomeworkAndStudent(
                            homeworkId,
                            studentId,
                        )
                    })
                ...
                if (homeworkSubmit) {
                    const pts =
                        homeworkSubmit.competitionSubmit?.totalPoints ??
                        homeworkSubmit.points
                    const maxPts =
                        homeworkSubmit.competitionSubmit?.maxPoints
