...
export class CreateClassStudentUseCase {
  ...
  async execute(createDto: CreateClassStudentDto, adminId?: number): Promise<BaseResponseDto<ClassStudentResponseDto>> {
    if (!createDto.studentId) {
      throw new ConflictException(`ID học sinh không được để trống`)
    }

    // Check if course class exists
    const courseClass = await this.courseClassRepository.findById(createDto.classId)
    if (!courseClass) {
    ...
    const enrollment = await this.enrollmentRepository.findByCourseAndStudent(courseClass.courseId, createDto.studentId)

    if (!enrollment) {
      throw new ConflictException(`Học sinh chưa đăng ký khóa học của lớp học này`)
    }

    // Check if class student already exists
    const existingClassStudent = await this.classStudentRepository.exists(createDto.classId, createDto.studentId)
    if (existingClassStudent) {
      throw new ConflictException(`Học sinh đã được thêm vào lớp học này rồi`)
    }
    ...
    return new BaseResponseDto(true, 'Thêm học sinh vào lớp học thành công', classStudentDto)
  }
}
