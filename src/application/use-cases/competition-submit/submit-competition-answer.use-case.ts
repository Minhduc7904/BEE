...
export class SubmitCompetitionAnswerUseCase {
  ...
  async execute(
    ...
    if (durationMinutes !== null && submit.startedAt) {
    ...
    if (question.type === QuestionType.TRUE_FALSE && body.trueFalseAnswers) {
      ...
      selectedStatementIds = body.trueFalseAnswers.filter((a) => a.isTrue === true).map((a) => a.statementId)
    ...
    const answerText = trueFalseAnswerJson ?? ('answer' in body ? body.answer : existingAnswer.answer)
    const savedAnswer = await this.competitionAnswerRepository.update(
      answerId,
      {
    ...
    if (delta !== 0) {
      ...
      await this.competitionSubmitRepository.update(
        submitId,
        {
  ...
  private gradeAnswer(
    ...
    switch (type) {
      ...
      case QuestionType.SINGLE_CHOICE: {
        const correctIds = (question.statements ?? []).filter((s: any) => s.isCorrect).map((s: any) => s.statementId)
        const isCorrect =
      ...
      case QuestionType.TRUE_FALSE: {
        ...
        const gradedStatements =
          answeredSet !== null ? statements.filter((s: any) => answeredSet.has(s.statementId)) : statements
        ...
        const correctCount = gradedStatements.filter((s: any) => s.isCorrect === selectedSet.has(s.statementId)).length
