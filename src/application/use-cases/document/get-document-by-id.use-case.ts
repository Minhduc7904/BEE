...
export class GetDocumentByIdUseCase {
  ...
  async execute(documentId: number): Promise<BaseResponseDto<DocumentResponseDto>> {
    const document = await this.unitOfWork.executeInTransaction((repos) =>
      repos.documentRepository.findById(documentId, true),
    )
  ...
  private async attachProcessedContent(response: DocumentResponseDto): Promise<void> {
    if (!response.content) {
      ...
      response.processedMarkdownContent = response.content
      return
    }
    ...
    const [processedMarkdownResults, processedHtmlResults] = await Promise.all([
      this.processContentUseCase.execute(contentFields, DOCUMENT_MEDIA_URL_EXPIRY_SECONDS),
      this.processContentAndRenderHtmlUseCase.execute(contentFields, DOCUMENT_MEDIA_URL_EXPIRY_SECONDS),
    ])

    response.processedMarkdownContent =
      this.processContentUseCase.getProcessedContent(
        processedMarkdownResults,
        DOCUMENT_MEDIA_FIELDS.CONTENT,
      ) || response.content

    response.processedContent =
      this.processContentAndRenderHtmlUseCase.getProcessedContent(
        processedHtmlResults,
        DOCUMENT_MEDIA_FIELDS.CONTENT,
      ) || response.content
  }
