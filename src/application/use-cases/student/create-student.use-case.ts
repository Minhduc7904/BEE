...
import { CreateStudentDto, StudentResponseDto, BaseResponseDto } from '../../dtos'
...
import { AttendanceStatus, CourseEnrollmentStatus, CourseEnrollmentType, StudentType } from 'src/shared/enums'
    ...
    return this.unitOfWork.executeInTransaction(async (repos) => {
      ...
      if (dto.classIds && dto.classIds.length > 0) {
        ...
        if (dto.courseIds?.length) {
          ...
          const invalidClasses = courseClasses.filter((c) => !courseIds.includes(c.courseId))
      ...
      return BaseResponseDto.success('Tạo học sinh thành công', StudentResponseDto.fromStudentEntity(studentWithUser))
    })
  }
}
