...
export class SeoMediaSlotMapper {
  static toDomain(
    ...
    return new SeoMediaSlotEntity({
      ...
      pageKey: prismaSlot.pageKey ?? null,
      type: prismaSlot.type ?? 'image',
      description: prismaSlot.description ?? null,
      isActive: prismaSlot.isActive ?? true,
      minItems: prismaSlot.minItems ?? 0,
      maxItems: prismaSlot.maxItems ?? null,
      recommendedWidth: prismaSlot.recommendedWidth ?? null,
      recommendedHeight: prismaSlot.recommendedHeight ?? null,
      metadata: prismaSlot.metadata ?? null,
