...
import { StudentLatestLessonsQueryDto } from '../../application/dtos/lesson/student-latest-lessons-query.dto'
...
import {
  ...
  GetStudentLatestLessonsUseCase,
...
export class LessonController {
  constructor(
    ...
    private readonly getStudentLatestLessonsUseCase: GetStudentLatestLessonsUseCase,
   ...
   * - Student có enrollment ACTIVE, hoặc course online PUBLISHED cho phép preview.
   * - Chỉ trả về lesson có visibility = PUBLISHED.
   * - Lesson có visibility = DRAFT hoặc PRIVATE sẽ không được trả về.
   * - Lesson phải đi qua isVisible, availableFrom, availableUntil của class.
   *
   * Input:
  ...
  async getStudentCourseLessons(
    ...
    return ExceptionHandler.execute(() => this.getStudentCourseLessonsUseCase.execute(courseId, studentId))
  }

  /**
   * Latest lessons visible to the current student across active course enrollments.
   * GET /lessons/student/latest
   * Chỉ trả lesson PUBLISHED mà student được xem qua class, cùng rule với
   * GET /lessons/student/course/:courseId.
   */
  @Get('student/latest')
  @RequirePermission()
  @HttpCode(HttpStatus.OK)
  async getStudentLatestLessons(
    @Query() query: StudentLatestLessonsQueryDto,
    @CurrentUser('studentId') studentId: number,
  ): Promise<PaginationResponseDto<StudentLessonResponseDto>> {
    return ExceptionHandler.execute(() => this.getStudentLatestLessonsUseCase.execute(studentId, query))
  }
   ...
   * - Lesson phải đi qua isVisible, availableFrom, availableUntil của class,
   *   cùng rule với GET /lessons/student/course/:courseId.
   *
   * Input:
  ...
  async getStudentLessonById(
    ...
    return ExceptionHandler.execute(() => this.getStudentLessonByIdUseCase.execute(lessonId, studentId))
  }
  ...
  async createLesson(@Body() dto: CreateLessonDto): Promise<BaseResponseDto<LessonResponseDto>> {
  ...
  @HttpCode(HttpStatus.OK)
  async deleteLesson(@Param('id', ParseIntPipe) id: number): Promise<BaseResponseDto<null>> {
