...
export class OnlineCourseInvoicePaymentStatusController {
  constructor(
    ...
    private readonly deleteOnlineCourseInvoiceUseCase: DeleteOnlineCourseInvoiceUseCase,
  ) {}
  ...
  @HttpCode(HttpStatus.OK)
  async deleteOnlineCourseInvoice(@Param('invoiceId', ParseIntPipe) invoiceId: number) {
    return ExceptionHandler.execute(() => this.deleteOnlineCourseInvoiceUseCase.execute(invoiceId))
  }
  ...
  @RequirePermission()
  @HttpCode(HttpStatus.OK)
  async getPaymentStatus(@Param('invoiceId', ParseIntPipe) invoiceId: number, @CurrentUser('userId') userId: number) {
    return ExceptionHandler.execute(() => this.getOnlineCourseInvoicePaymentStatusUseCase.execute(invoiceId, userId))
  }
}
